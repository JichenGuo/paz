#!/usr/bin/env python
"""Run inference with the best RF-DETR Large checkpoint from Experiment 11."""
...
DEFAULT_CHECKPOINT = _SCRIPT_DIR / "checkpoints" / "rfdetr_large_best.weights.h5"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Load the Experiment 11 best RF-DETR Large checkpoint and run "
...
def make_output_paths(input_path, image_paths, args):
    ...
    if folder_mode:
        if args.output is None:
            output_dir = input_path / "experiment_11_detections"
        else:
        ...
        output_path = (
            image_path.with_name(f"{image_path.stem}_experiment_11_detected.jpg")
            if args.output is None
...
def build_model(checkpoint_path, class_names):
    if not checkpoint_path.exists():
        raise FileNotFoundError(
            ...
            "Train experiment_11 first or pass --checkpoint /path/to/model.weights.h5"
        )
