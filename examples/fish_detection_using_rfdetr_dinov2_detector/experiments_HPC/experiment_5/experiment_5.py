...
def _patched_train_one_epoch(
    ...
    lr_multipliers=None, ema_m=None, grad_accum_steps=4,
    multi_scale_config=None, drop_path_schedule=None, dropout_schedule=None,
    vit_encoder_num_layers=None, amp=False,
):
    """Patched ``train_one_epoch`` — uses ``training=True`` in Phase 1.

    Only change vs. original: line marked [PATCHED].
        New parameters (accepted but not used in this simple patch):
        lr_multipliers: differential learning rate multipliers
        ema_m: exponential moving average model
        grad_accum_steps: gradient accumulation steps
        multi_scale_config: multi-scale training config
        drop_path_schedule: drop path decay schedule
        dropout_schedule: dropout schedule
    ...
    for step, (images, targets) in enumerate(
        ...
        if "enc_outputs" in outputs_eager:
            enc_indices = criterion.matcher(
                outputs_eager["enc_outputs"], targets, group_detr=group_detr
            )
...
def main():
    ...
    logger.info("Train loader: %d batches of %d (pipeline2 + ImageNet norm)",
                len(train_loader), BATCH_SIZE*GRAD_ACCUM_STEPS)
