#!/usr/bin/env python
"""Extract finetuned RF-DETR backbone features and visualize them with t-SNE.
...
DEFAULT_OUTPUT_DIR = _SCRIPT_DIR / "feature_tsne_results"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "t-SNE to inspect class clustering."
        )
    )
    ...
    parser.add_argument(
        ...
        default="gt_boxes",
        help="What each t-SNE point represents.",
    )
    parser.add_argument(
        "--class-names",
        default="fish",
...
def run_tsne(features, perplexity, random_state, n_components):
    try:
        ...
        raise ImportError(
            "scikit-learn is required for t-SNE. Install it in your ML "
            "environment, e.g. `pip install scikit-learn`."
        ) from exc

    if len(features) < 3:
        raise ValueError("Need at least 3 feature points for t-SNE")
    ...
    return tsne.fit_transform(scaled), perplexity
...
def main():
    ...
    plot_path = output_dir / "detector_features_tsne.png"
    summary_path = output_dir / "detector_features_tsne_summary.json"
    ...
    plot_tsne(plot_path, embeddings, metadata)
