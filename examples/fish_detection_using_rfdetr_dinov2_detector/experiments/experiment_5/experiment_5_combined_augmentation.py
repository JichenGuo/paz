#!/usr/bin/env python
"""RF-DETR Nano training with combined DeepFish + native augmentation.

Based on the HPC Experiment 10 training script, but trains with a custom
loader that combines both augmentation strategies:

  1. Experiment 5 ``pipeline2`` augmentation:
     - RandomHorizontalFlip  (p = 0.5)
     - Brightness jitter     (p = 0.8, factor 0.7-1.3)
     - Contrast jitter       (p = 0.8, factor 0.7-1.3)
     - Saturation jitter     (p = 0.5, factor 0.7-1.3)

  2. Experiment 10 native RF-DETR augmentation
...
The combined loader applies ``pipeline2`` on the original DeepFish image,
converts the normalized cxcywh boxes to RF-DETR's native pixel xyxy target
format, applies ``make_coco_transforms_square_div_64("train", 384)``, and
then yields normalized image tensors plus normalized cxcywh boxes to
``train_from_config``.
...
from generator import _augment_pipeline2, prefetch_iterator
...
# =====================================================================
# Combined training loader: pipeline2 + native RF-DETR augmentation
# =====================================================================

def _target_cxcywh_norm_to_xyxy_pixels(target, image_shape):
    """Convert DeepFish normalized cxcywh targets to native pixel xyxy."""
    h, w = image_shape[:2]
    boxes = target["boxes"].astype(np.float32, copy=True)
    labels = target["labels"].astype(np.int64, copy=True)
    ...
    if len(boxes) > 0:
        ...
        area = np.maximum(boxes_xyxy[:, 2] - boxes_xyxy[:, 0], 0.0) * np.maximum(
...
class _CombinedAugmentationDataLoader:
    """Yield batches augmented by Experiment 5 pipeline2 and RF-DETR native transforms.

    ``train_from_config`` accepts any object with ``__len__`` and ``__iter__``
    yielding ``(images_np, targets)``. This loader keeps original DeepFish image
    sizes until the native RF-DETR transform performs the final 384x384 square
    resize/crop path, so variable-sized source images are supported.
    """
    ...
    def __init__(
        ...
        self._epoch = 0
        self._native_transform = make_coco_transforms_square_div_64(
            "train",
        ...
        if shuffle:
            self._shuffle_indices(seed)

    def __len__(self):
        return math.ceil(len(self.indices) / self.batch_size)

    def _shuffle_indices(self, seed):
    ...
    def set_epoch(self, epoch):
        self._epoch = epoch
        if self.shuffle:
            self._shuffle_indices(self.seed + epoch)

    def __iter__(self):
        epoch = self._epoch
        self.set_epoch(epoch)
        try:
            yield from prefetch_iterator(self, max_prefetch=self.max_prefetch)
        finally:
            self._epoch = epoch + 1
    ...
    def __getitem__(self, batch_idx):
        ...
        rng = np.random.RandomState(
            self.seed + self._epoch * 100000 + batch_idx
        )
        ...
        for idx in batch_indices:
            ...
            # Experiment 5 augmentation works on float HWC [0, 1] images and
            # normalized cxcywh boxes.
            image, target = _augment_pipeline2(image, target, rng)

            native_target = _target_cxcywh_norm_to_xyxy_pixels(
                target, image.shape
            )
            ...
            # RF-DETR native transforms use Python's random module.
            random.seed(int(rng.randint(0, 2**31 - 1)))
            image_aug, target_aug = self._native_transform(
                pil_image, native_target
            )

            images.append(image_aug.astype(np.float32, copy=False))
            targets.append({
                "boxes": target_aug["boxes"].astype(np.float32, copy=False),
                "labels": target_aug["labels"].astype(np.int64, copy=False),
            })

        return np.stack(images, axis=0).astype(np.float32), targets
...
class ExperimentTracker:
    ...
    def on_epoch_end(self, log_stats):
        ...
        if run_val_eval and val_loss < self.best_val_loss:
            ...
            best_path = os.path.join(self.ckpt_dir, "rfdetr_nano_best.weights.h5")
...
def main():
    ...
    logger.info("=" * 68)
    logger.info("RF-DETR Nano — Combined pipeline2 + Native Augmentation")
    logger.info("=" * 68)
    ...
    # Load dataset: resolution=None for combined training augmentation so the
    # native RF-DETR transform sees original image geometry. eval_ds uses 384
    # for validate_epoch_full, matching existing experiment scripts.
    ...
    train_loader = _CombinedAugmentationDataLoader(
        dataset=ds,
    ...
    logger.info(
        "Train loader: %d batches of %d (pipeline2 + native RF-DETR)",
    ...
    exp_config = {
        "experiment": "experiment_5_combined_augmentation",
        "description": (
            "RF-DETR Nano with combined Experiment 5 pipeline2 augmentation "
            "and Experiment 10 native RF-DETR augmentation; all other "
            "hyperparameters follow the Experiment 10 training script"
        ),
        "variant": "RFDETRNano",
        "api": "high-level (RFDETRNano.train_from_config)",
        "augmentation": "combined pipeline2 + native RF-DETR square_div_64",
        "augmentation_order": [
            "pipeline2: hflip + brightness/contrast/saturation jitter",
            "native RF-DETR: hflip + RandomSelect(SquareResize, ResizeCropSquareResize)",
        ...
        "vs_experiment_5": (
            "Keeps pipeline2 hflip/color jitter and adds native RF-DETR "
            "hflip + RandomSizeCrop + SquareResize."
        ),
        "vs_experiment_10": (
            "Keeps native RF-DETR augmentation and prepends pipeline2 "
            "hflip/color jitter through a custom train_from_config loader."
        ),
    }
    ...
    # Train with the combined custom loader. Validation still uses the
    # ExperimentTracker callback and the COCO data prepared above.
